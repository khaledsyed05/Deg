...
function toggleVenue(id: number) {
  const idx = form.venue_ids.indexOf(id)
  if (idx === -1) { form.venue_ids.push(id) } else { form.venue_ids.splice(idx, 1) }
}

function submit() {
  form.post('/club/promotions')
}
...
<template>
  <Head title="إضافة ترقية جديدة" />

  <ClubLayout>
    <div class="px-4 sm:px-6 py-6 sm:py-8 max-w-2xl">
      <div class="flex items-center gap-3 mb-6">
        <a href="/club/promotions" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <h1 class="text-xl font-bold text-gray-900 dark:text-white">إضافة ترقية جديدة</h1>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        <div class="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <input v-model="form.code" type="text" dir="ltr" placeholder="SUMMER20" :class="[inputClass, form.errors.code && 'border-red-400']" />
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="form.discount_value" type="number" min="0" dir="ltr" :class="[inputClass, form.errors.discount_value && 'border-red-400']" />
              <p v-if="form.errors.discount_value" class="text-xs text-red-500 mt-1">{{ form.errors.discount_value }}</p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            <input id="is_active" v-model="form.is_active" type="checkbox" class="w-4 h-4 rounded border-gray-300 text-emerald-500 focus:ring-emerald-400" />
            <label for="is_active" class="text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
              تفعيل الترقية فور الإنشاء
            </label>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          <button type="submit" :disabled="form.processing" class="px-6 py-2.5 text-sm font-semibold rounded-xl bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-white transition-colors shadow-sm">
            {{ form.processing ? 'جارٍ الحفظ...' : 'إنشاء الترقية' }}
          </button>
          <a href="/club/promotions" class="px-6 py-2.5 text-sm font-semibold rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            إلغاء
          </a>
        </div>
      </form>
