...
class SyriatelWalletGateway implements WalletTopupGatewayInterface
{
    public function initiate(Payment $payment, array $context): WalletTopupInitiationResult
    {
        ...
        $providerRef = 'WTU-SYR-'.$payment->id.'-'.bin2hex(random_bytes(4));

        Log::channel('payments')->info('wallet_topup.syriatel.initiate', [
        ...
        $response = Http::timeout(15)
            ...
            ->post(rtrim($config['base_url'], '/').'/payments/initiate', [
        ...
        if (! $response->successful()) {
            throw new \RuntimeException('فشل بدء الدفع عبر Syriatel Cash');
        }
    ...
    public function confirm(Payment $payment, string $otp): WalletTopupConfirmResult
    {
        ...
        Log::channel('payments')->info('wallet_topup.syriatel.confirm', ['payment_id' => $payment->id]);

        $response = Http::timeout(15)
            ->withToken($config['api_key'])
            ->post(rtrim($config['base_url'], '/').'/payments/confirm', [
                'merchant_code' => $config['merchant_code'],
    ...
    public function resendOtp(Payment $payment): bool
    {
        ...
        $response = Http::timeout(15)
            ->withToken($config['api_key'])
            ->post(rtrim($config['base_url'], '/').'/payments/resend-otp', [
                'merchant_code' => $config['merchant_code'],
                'reference' => $payment->provider_reference,
            ]);
    ...
    {
        return new WalletTopupConfirmResult(success: false, errorMessage: 'Syriatel uses OTP confirm, not webhook');
    }

    /**
     * @return array{base_url: string, api_key: string, merchant_code: string}
     */
    private function config(): array
    {
        $base = (string) config('payments.syriatel.base_url');
        $key = (string) config('payments.syriatel.api_key');
        $merchant = (string) config('payments.syriatel.merchant_code');

        if ($base === '' || $key === '' || $merchant === '') {
            throw new \RuntimeException('Syriatel Cash لم يُكوَّن (راجع PAYMENT_SYRIATEL_* في .env)');
        }

        return ['base_url' => $base, 'api_key' => $key, 'merchant_code' => $merchant];
    }
